...
Download the [project template](https://github.com/Myaamori/SubKt/releases/download/0.1.6/subkt-template-0.1.6.zip) and place the files in the directory where you keep your project files.
...
The key here is the use of `mux.items()`.
While `mux.item()` called from e.g. `torrent.vol1` would return the (non-existent) task `mux.vol1`, `items()` instead returns all tasks belonging to the current batch---in `torrent.vol1` this means `mux.01` and `mux.02`.
If called from an episode task, `items()` will return the same tasks as `item()`, since an episode task only corresponds to one single episode.
For instance, if you were to call `mux.items()` from `torrent.01`, you would get a list containing a single task: `[mux.01]`.
...
Here, we rely on the fact that `mux.items()` returns the same task as `mux.item()` for episode tasks.
Note also the use of `torrent.item()`---not `torrent.items()`---in the `nyaa` task, since in e.g. `nyaa.vol1` we want to upload the torrent generated by `torrent.vol1`, not the two torrents generated by `torrent.01` and `torrent.02`.
...
Using the `value` function, you can define a group of values---just like how the `task` function defines a group of tasks---which can then be accessed using the `item()` and `items()` functions, just like task groups.
