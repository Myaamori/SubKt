...
| [SFTP](-s-f-t-p/index.md) | `abstract class SFTP : `[`AbstractTransferTask`](-abstract-transfer-task/index.md)`<ChannelSftp>`<br>Task for uploading files via SFTP (SSH). A predefined task instance can be accessed through [Subs.sftp](sftp.md). |
...
| [globPath](glob-path.md) | `fun globPath(glob: `[`String`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-string/index.html)`): `[`List`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/-list/index.html)`<`[`Path`](https://docs.oracle.com/javase/9/docs/api/java/nio/file/Path.html)`>`<br>Finds paths matching the given string. `%` and `*` are interpreted as wildcards. |
